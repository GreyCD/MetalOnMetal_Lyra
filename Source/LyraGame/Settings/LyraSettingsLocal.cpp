...
ULyraSettingsLocal::ULyraSettingsLocal()
{
	...
	//OnAllModuleLoadingPhasesCompleteHandle = FCoreDelegates::OnAllModuleLoadingPhasesComplete.AddUObject(this,  &ThisClass::OnAllModuleLoadingPhasesComplete);
	SetToDefaults();
}
...
// void ULyraSettingsLocal::OnAllModuleLoadingPhasesComplete()
// {
// 	ReapplyThingsDueToPossibleDeviceProfileChange();
// }

/*void ULyraSettingsLocal::ApplyDLSSMode() const
{
	constexpr bool bRestoreFullResWhenDisabled = true;
	const bool bEnableDLSS = DLSSMode != UDLSSMode::Off;
	UDLSSLibrary::EnableDLSS(bEnableDLSS);

	const FVector2D ScreenResolution = GetScreenResolution();
	bool bIsSupported;
	...
	const bool bUseOptimalScreenPercentage = DLSSMode != UDLSSMode::Off && DLSSMode != UDLSSMode::DLAA;
	const float ScreenPercentage = bUseOptimalScreenPercentage ? OptimalScreenPercentage : 100.0f;
	UKismetSystemLibrary::ExecuteConsoleCommand(GetWorld(), TEXT("r.ScreenPercentage ") + FString::SanitizeFloat(ScreenPercentage));
...
void ULyraSettingsLocal::ApplyNonResolutionSettings()
{
	...
	if (FApp::CanEverRender())
	{
		...
		//ProjectOverrides start: Add NVIDIA support
		//if (FModuleManager::Get().IsModuleLoaded(TEXT("DLSS")))        // @TODO: Probably do a less lazy check
		//{
			//ApplyUpscaleMode();
			//ApplyDLSSMode();
			//ApplyNISMode();
