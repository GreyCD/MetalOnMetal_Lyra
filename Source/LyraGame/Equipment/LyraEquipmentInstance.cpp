...
//@TODO: bringfire games - Further implementation of this function is required
AActor* ULyraEquipmentInstance::GetFirstSpawnedActor() const
{
...
void ULyraEquipmentInstance::SpawnEquipmentActors(const TArray<FLyraEquipmentActorToSpawn>& ActorsToSpawn)
{
	if (APawn* OwningPawn = GetPawn())
	{
		...
		for (const FLyraEquipmentActorToSpawn& SpawnInfo : ActorsToSpawn)
		{
			...
			NewActor->AttachToComponent(AttachTarget, FAttachmentTransformRules::SnapToTargetNotIncludingScale, SpawnInfo.AttachSocket);
			
			// Confirm actor has been spawned and attached
			UE_LOG(LogTemp, Log, TEXT("Spawned %s, attaching to %s at socket %s"), 
				*GetNameSafe(NewActor), 
				*GetNameSafe(AttachTarget), 
				*SpawnInfo.AttachSocket.ToString());

			// Assuming SpawnedActors is an array property of ULyraEquipmentInstance
			SpawnedActors.Add(NewActor);
			//UE_LOG(LogTemp, Log, TEXT("Added %s to SpawnedActors"), *GetNameSafe(NewActor));
