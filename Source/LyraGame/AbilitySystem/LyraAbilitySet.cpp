...
void FLyraAbilitySet_GrantedHandles::TakeFromAbilitySystem(ULyraAbilitySystemComponent* LyraASC)
{
	...
	for (const FGameplayAbilitySpecHandle& Handle : AbilitySpecHandles)
	{
		if (Handle.IsValid())
		{
			// StartChange // Thx NAMROG84 for share this
			if (const FGameplayAbilitySpec* AbilitySpec = LyraASC->FindAbilitySpecFromHandle(Handle))
			{
				...
				if(ULyraGameplayAbility* Ability = Cast<ULyraGameplayAbility>(Instance)) 
				{
					Ability->K2_OnAbilityRemoved();
				}
			}
