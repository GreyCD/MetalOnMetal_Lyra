...
void ULyraCombatSet::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME_CONDITION_NOTIFY(ULyraCombatSet, BaseArmor, COND_OwnerOnly, REPNOTIFY_Always);
}
...
void ULyraCombatSet::OnRep_BaseArmor(const FGameplayAttributeData& OldValue)
{
	GAMEPLAYATTRIBUTE_REPNOTIFY(ULyraCombatSet, BaseArmor, OldValue);
}
