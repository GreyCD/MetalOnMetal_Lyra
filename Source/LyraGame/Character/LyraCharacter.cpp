...
#include "LyraOnMetalStatsComponent.h"
	...
	: Super(ObjectInitializer.SetDefaultSubobjectClass<ULyraCharacterMovementComponent>(ACharacter::CharacterMovementComponentName))
{
	...
	OnMetalStatsComponent = CreateDefaultSubobject<ULyraOnMetalStatsComponent>(TEXT("OnMetalStatsComponent"));

	//CameraComponent = CreateDefaultSubobject<ULyraCameraComponent>(TEXT("CameraComponent"));
	//CameraComponent->SetRelativeLocation(FVector(-300.0f, 0.0f, 75.0f));

	// In the LyraCharacter constructor

	// ...
	CameraComponent = CreateDefaultSubobject<ULyraCameraComponent>(TEXT("FPSCamera"));
	CameraComponent->SetupAttachment(MeshComp, TEXT("S_Camera")); // Attach the camera to the "S_Camera" socket
	CameraComponent->SetRelativeLocation(FVector::ZeroVector); // Set the relative location to zero since it will be positioned by the socket
	CameraComponent->SetRelativeRotation(FRotator::ZeroRotator); // Set the relative rotation to zero since it will be oriented by the socket
	//CameraComponent->ShiftLens = 0.2f;
	// ...
...
void ALyraCharacter::OnAbilitySystemInitialized()
{
	...
	OnMetalStatsComponent->InitializeWithAbilitySystem(LyraASC);
...
void ALyraCharacter::OnAbilitySystemUninitialized()
{
	...
	OnMetalStatsComponent->UninitializeFromAbilitySystem();
}
...
void ALyraCharacter::UpdateMovementSpeed()
{
	if (ULyraCharacterMovementComponent* LyraMovementComponent = Cast<ULyraCharacterMovementComponent>(GetCharacterMovement()))
	{
		if (GetAbilitySystemComponent()->HasMatchingGameplayTag(FGameplayTag::RequestGameplayTag("Event.Movement.Sprinting")))
		{
			//LyraMovementComponent->MaxWalkSpeed = LyraMovementComponent->SprintSpeed;
			LyraMovementComponent->SetSprintingState(true);
		}
		else
		{
			//LyraMovementComponent->MaxWalkSpeed = LyraMovementComponent->DefaultMaxWalkSpeed;
			LyraMovementComponent->SetSprintingState(false);
		}
	}
}
