...
IMPLEMENT_STRUCT_SERIALIZATION_TEST_NEW(TestCategory, StructType, StructInstance, EAutomationTestFlags::ApplicationContextMask | Flags)
...
namespace \
{ \
	CREATE_SERIALIZATION_TEST_NAME(StructType) PREPROCESSOR_JOIN(PREPROCESSOR_JOIN(StructType, Serialization), AutomationTestInstance)( PREPROCESSOR_JOIN( L, PREPROCESSOR_TO_STRING( CREATE_SERIALIZATION_TEST_NAME(StructType) ) ) ); \
}; \
bool CREATE_SERIALIZATION_TEST_NAME(StructType)::RunTest(const FString& Parameters) \
{ \
	bool bPassed = true;
