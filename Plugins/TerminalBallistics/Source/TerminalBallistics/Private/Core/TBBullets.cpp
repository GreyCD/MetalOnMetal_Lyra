...
namespace TB::Bullets
{
	...
	{
		constexpr double Radius300BLK = InToCm(0.308 / 2.0);
		constexpr double Radius300WinMag = InToCm(0.308 / 2.0);
		...
		const FTBOgive Ogive_300WinMag =	FTBOgive(Radius300WinMag,	7.626,			1.763,		9.358e-5,		FTBCylinder(Radius300WinMag,	1.4122));
		...
		const FTBOgive Ogive_380ACP =		FTBOgive(0.452755,							0.628,		4.464e-5,		FTBCylinder(0.452755,			0.558475));
	};
	...
	namespace BulletPhysicalProperties
	{
		...
		constexpr double Mass300WinMag = GrainsToKilograms(168.0);
		...
		using BulletOgives::Radius300WinMag;
		...
		const FTBBulletPhysicalProperties BULLET_300WinMag =	FTBBulletPhysicalProperties(Mass300WinMag,	3.1775,		8.859,				Radius300WinMag,	0.00020178,		0.2238,		0.5884,		-1.0,		10.25,	0.260,	0.253, G7, BulletOgives::Ogive_300WinMag); // Berger 168gr VLD
	...
	void BulletTypes::Init()
	{
		...
		StaticBulletTypes.Bullet_300WinMag =	FTBBullet(BULLET_300WinMag, Lead, BulletName300WinMag, ETBBulletCaliber::Bullet_300WINMAG, PS_Medium, 980, FMJ_BulletVariation);
		...
		StaticBulletTypes.Bullet_45ACP =		FTBBullet(BULLET_45ACP, Lead, BulletName45ACP, ETBBulletCaliber::Bullet_45ACP, PS_Medium, 263.5, FMJ_BulletVariation);
		...
		StaticBulletTypes.Bullet_12Gauge00Buckshot = FTBBullet(BULLET_12Gauge00Buck, Lead, BulletName12Gauge00Buckshot, ETBBulletCaliber::Bullet_12Gauge, PS_Medium, 373.38, ETBBulletVariation::BUCKSHOT, 1.f, Buckshot00::Count_3in, Buckshot00::SpreadParams);
	...
	bool BulletTypes::IsValid() const
	{
		auto ValidityPredicate = [](const TPair<ETBBulletNames, FTBBullet>& Pair)
		{
			return Pair.Value.IsValid();
		};
		return BulletMappings.Num() > 0 && BulletMappings.FilterByPredicate(ValidityPredicate).Num() > 0;
	}
...
FTBBullet FTBBullet::None = FTBBullet();
...
FTBBullet::FTBBullet(const UBulletDataAsset* DataAsset)
	...
	, GameplayTag(DataAsset->GameplayTag)
...
{
	if (DataAsset->bUseExistingBullet && !DataAsset->bCustomizePreset)
	{
		const auto Mappings = TB::Bullets::BulletTypes::Get().GetMappings();
		if (const auto FoundBulletProperties = Mappings.Find(DataAsset->PresetBullets))
		{
			BulletName = FoundBulletProperties->BulletName;
			GameplayTag = FoundBulletProperties->GameplayTag;
			BulletProperties = FoundBulletProperties->BulletProperties;
			BulletType = FoundBulletProperties->BulletType;
			BulletVariation = FoundBulletProperties->BulletVariation;
			MuzzleVelocity = FoundBulletProperties->MuzzleVelocity;
			PhysicalProperties = FoundBulletProperties->PhysicalProperties;
			ProjectileCount = FoundBulletProperties->ProjectileCount;
			ProjectileSize = FoundBulletProperties->ProjectileSize;
			SpreadParams = FoundBulletProperties->SpreadParams;
		}
	}
}
