...
#include "Types/TBCollisionParams.h"
...
#include "Types/TBOverlapArray.h"
...
namespace TB
{
		...
		else if (const AActor* HitActor = hitResult.GetActor())
		{
			...
			else if (hitResult.GetActor()->IsA<APawn>() || hitResult.GetComponent()->GetCollisionObjectType() == ECC_Pawn) // no tags, but it's a pawn
			{
				...
				if (hitResult.PhysMaterial.IsValid())
				{
					...
					const FPhysMatProperties PhysMatProperties = UTBPhysMatManager::Get().GetFromHitResult(hitResult);
					isFlesh = surfaceType == SurfaceTypeFlesh || TB::PhysMatHelpers::IsFlesh(PhysMatProperties);
					isBone = surfaceType == SurfaceTypeBone || TB::PhysMatHelpers::IsBone(PhysMatProperties);
				}
			}
		}
	...
	FPhysMatProperties GetSurfaceProperties(FHitResult& HitResult, bool* IsHitZone, bool* IsBone, bool* IsDead, bool* IsFlesh)
	{
		...
		if (TB::ImplementsDynamicMaterialPropertyManager(HitResult.GetComponent()))
		{
			const auto& Properties = ITBDynamicMaterialPropertyManager::Execute_GetMaterialProperties(HitResult.GetComponent(), HitResult);
			bIsBone |= TB::PhysMatHelpers::IsBone(Properties);
			bIsFlesh |= TB::PhysMatHelpers::IsFlesh(Properties);
			bIsHitZone |= bIsFlesh || bIsBone;
			SetOutputPtrs();
			return Properties;
		}
		else if (TB::ImplementsDynamicMaterialPropertyManager(HitResult.GetActor()))
		{
			const auto& Properties = ITBDynamicMaterialPropertyManager::Execute_GetMaterialProperties(HitResult.GetActor(), HitResult);
			bIsBone |= TB::PhysMatHelpers::IsBone(Properties);
			bIsFlesh |= TB::PhysMatHelpers::IsFlesh(Properties);
			bIsHitZone |= bIsFlesh || bIsBone;
			SetOutputPtrs();
	...
	bool SweepSingle(const UWorld* World, const FCollisionShape& CollisionShape, const FQuat& Rot, FHitResult& OutHit, FVector Start, FVector End, ECollisionChannel TraceChannel, const FCollisionQueryParams& Params, const FCollisionResponseParams& ResponseParams, const FCollisionObjectQueryParams& ObjectParams)
	{
		...
		{
			return FPhysicsInterface::RaycastSingle(World, OutHit, Start, End, TraceChannel, Params, ResponseParams, ObjectParams);
		}
		else
		{
	...
	bool DoesShapeOverlap(const FHitResult& HitResult, const FVector& Location, const FCollisionShape& CollisionShape, const FQuat& Rot, const int32 PreviousShapeIndex)
	{
		...
		if(const USkeletalMeshComponent* SkeletalMeshComponent = Cast<USkeletalMeshComponent>(HitComponent))
		{
			...
			{
				BoneName = SkeletalMeshComponent->FindClosestBone(HitResult.Location, nullptr, 0, true);
			}

			const int32 ShapeIndex = GetShapeIndex(SkeletalMeshComponent->GetBodyInstance(SkeletalMeshComponent->FindClosestBone(Location, nullptr, 0, true)), Location);
	...
	bool FindExitFromSkeletalMeshComponent(FPath Path, USkeletalMeshComponent* SkeletalMeshComponent, const double Radius, FExitResult& ExitResult, bool bTraceComplex, ECollisionChannel TraceChannel)
	{
		...
		End = Box.GetClosestPointTo(End) + Path.GetDirectionVector() * (2.0 * Radius); // Nudge forward a bit to help with depenetration


		/* 
			Since the body instance may be made up of separate shapes, we may end thinking the shape was thicker than it actually is
			...
			Instead, we'll have to find the actual shape we hit, and then find the exit from there.
			Luckily, this will also capture the most accurate exit point.
		*/
		{
			const FVector PointInsideShape = BodyInstance->GetUnrealWorldTransform().InverseTransformPosition(Path.PathStart + Path.GetDirectionVector() * (2.0 * Radius));
			const FVector EndLocal = BodyInstance->GetUnrealWorldTransform().InverseTransformPosition(End);
			TRACE_CPUPROFILER_EVENT_SCOPE(TB::FindExitFromSkeletalMeshComponent::ShapeSearch);
			FScopedSceneLock_Chaos SceneLock(SkeletalMeshComponent, EPhysicsInterfaceScopedLockType::Read);
			TArray<FPhysicsShapeHandle> Shapes;
			const int32 NumShapes = BodyInstance->GetAllShapes_AssumesLocked(Shapes);
			if (NumShapes >= 0)
			{
				FPhysicsShapeHandle ClosestShape = Shapes[0];
				int32 ClosestShapeIndex = 0;
				if (NumShapes > 1)
				{
					double ShapeDist = INFINITY;
					for (int i = 0; i < NumShapes; i++)
					{
						Chaos::FVec3 norm;
						const double dist = Shapes[i].GetGeometry().PhiWithNormal(Path.PathStart, norm);
						if (dist < ShapeDist)
						{
							ShapeDist = dist;
				...
				const auto result = Shapes[ClosestShapeIndex].GetGeometry().FindClosestIntersection(PointInsideShape, EndLocal, Radius);
				if (result.Second)
				{
					ExitResult.ShapeIndex = ClosestShapeIndex;
					End = BodyInstance->GetUnrealWorldTransform().TransformPosition(result.First);
				}
			}
		}
		...
		if (ExitFound)
		{
			FMTDResult MTD = FMTDResult();
			if (BodyInstance->OverlapTest(ExitResult.ExitHitResult.Location, FQuat::Identity, Shape, &MTD, true))
			{
				ExitResult.NudgeExit(MTD.Direction, MTD.Distance + 0.1);
			}
			ExitResult.NudgeExit(Path.GetDirectionVector(), 2.5 * Radius);
			ExitResult.CalcExitVarsFromExitResult();
			return true;
		}
		else
	...
	int32 GetShapeIndex(FBodyInstance* BodyInstance, const FVector& Location)
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(TB::GetShapeIndex);
		int32 ClosestShapeIndex = -1;
		const FVector PointInLocalSpace = BodyInstance->GetUnrealWorldTransform().InverseTransformPosition(Location);
		FScopedSceneLock_Chaos SceneLock(&BodyInstance->GetPhysicsActorHandle(), EPhysicsInterfaceScopedLockType::Read);
		TArray<FPhysicsShapeHandle> Shapes;
		const int32 NumShapes = BodyInstance->GetAllShapes_AssumesLocked(Shapes);
		if (NumShapes >= 0)
		{
			...
			if (NumShapes > 1)
			{
				...
				{
					Chaos::FVec3 norm;
					const double dist = Shapes[i].GetGeometry().PhiWithNormal(PointInLocalSpace, norm);
					if (dist < ShapeDist)
					{
						ShapeDist = dist;
						ClosestShapeIndex = i;
	...
	FExitResults GetExitLocation(const UObject* WorldContextObject, const FHitResult& HitResult, const double Radius, bool& bExitFound, bool& bMultipleHits, ECollisionChannel TraceChannel)
	{
		...
		if(Results.Num() == 0)
		{
			TB_LOG(Error, TEXT("No Exits Found!"));
		}
		return Results;
	}
};
...
FTBLaunchParams UTerminalBallisticsStatics::MakeLaunchParams(
	...
	double TracerActivationDistance,
	UObject* Payload
)
{
	...
	return FTBLaunchParams(Owner, Instigator, ProjectileSpeed, EffectiveRange, FTBLocationAndDirection(FireLocation, FireDirection), ToIgnore, ObjectTypes, TraceChannel, bIgnoreOwner, bAddToOwnerVelocity, bForceNoTracer, Timescale, GravityMultiplier, OwnerIgnoreDistance, TracerActivationDistance, Payload);
}
...
)
{
	...
	GravityMultiplier = LaunchParams.GravityMultiplier;
	OwnerIgnoreDistance = LaunchParams.OwnerIgnoreDistance;
	TracerActivationDistance = LaunchParams.TracerActivationDistance;
...
FTBProjectileId UTerminalBallisticsStatics::AddAndFireBullet(TSoftObjectPtr<class UBulletDataAsset> Bullet, FTBLaunchParams& LaunchParams, const FTBProjectileId& Id, int32 DebugType)
{
	if (UTerminalBallisticsSubsystem* TBSubsystem = FTBGetSubsystemHelper::GetTBSubsystem(LaunchParams.Owner))
	{
		...
		{
			return TBSubsystem->AddAndFireBullet(BulletDataAsset, LaunchParams, Id, DebugType);
		}
	}
...
)
{
	if (UTerminalBallisticsSubsystem* TBSubsystem = FTBGetSubsystemHelper::GetTBSubsystem(LaunchParams.Owner))
	{
		...
		if (TB_VALID(BulletDataAsset))
		{
			return TBSubsystem->AddAndFireBulletWithCallbacks(BulletDataAsset, LaunchParams, OnBulletComplete, OnBulletHit, OnBulletExitHit, OnBulletInjure, Id, DebugType);
		}
	}
	return FTBProjectileId::None;
}
...
)
{
	if (UTerminalBallisticsSubsystem* TBSubsystem = FTBGetSubsystemHelper::GetTBSubsystem(LaunchParams.Owner))
	{
		...
		if (TB_VALID(BulletDataAsset))
		{
			return TBSubsystem->AddAndFireBulletWithCallbacksAndUpdate(BulletDataAsset, LaunchParams, OnBulletComplete, OnBulletHit, OnBulletExitHit, OnBulletInjure, OnUpdate, Id, DebugType);
		}
	}
	return FTBProjectileId::None;
}

TArray<FTBProjectileId> UTerminalBallisticsStatics::AddAndFireMultiProjectileBullet(TSoftObjectPtr<UBulletDataAsset> Bullet, FTBLaunchParams& LaunchParams, int32 DebugType)
{
	if (UTerminalBallisticsSubsystem* TBSubsystem = FTBGetSubsystemHelper::GetTBSubsystem(LaunchParams.Owner))
	{
		UBulletDataAsset* BulletDataAsset = Bullet.LoadSynchronous();
		if (TB_VALID(BulletDataAsset))
		{
			const int32 Num = BulletDataAsset->ProjectileCount;
			TArray<FTBProjectileId> Ids;
			Ids.AddUninitialized(Num);
			ParallelFor(Num, [&](int32 i)
				{
					FTBLaunchParams LocalLaunchParams = LaunchParams;
					LocalLaunchParams.FireTransform.Direction = BulletDataAsset->SpreadParams.GenerateSpreadVector(LaunchParams.FireTransform.Direction);
					Ids[i] = TBSubsystem->AddAndFireBullet(BulletDataAsset, LocalLaunchParams, FTBProjectileId::CreateNew(), DebugType);
				}
			);
		}
	}
	return TArray<FTBProjectileId>();
}

FTBProjectileId UTerminalBallisticsStatics::AddAndFireProjectile(
...
)
{
	if (UTerminalBallisticsSubsystem* TBSubsystem = FTBGetSubsystemHelper::GetTBSubsystem(LaunchParams.Owner))
	{
		...
		simData.SetId(Id);
		return TBSubsystem->AddAndFireProjectile(simData, LaunchParams, DebugType);
	}
	return FTBProjectileId::None;
}
...
)
{
	if (UTerminalBallisticsSubsystem* TBSubsystem = FTBGetSubsystemHelper::GetTBSubsystem(LaunchParams.Owner))
	{
		FTBProjectileSimData simData = FTBProjectileSimData(Projectile, ProjectilePhysicalProperties, DebugType);
		simData.SetId(Id);
		TBSubsystem->AddAndFireProjectileWithCallbacks(simData, LaunchParams, OnProjectileComplete, OnProjectileHit, OnProjectileExitHit, OnProjectileInjure, DebugType);
	}
	return FTBProjectileId::None;
}
...
)
{
	if (UTerminalBallisticsSubsystem* TBSubsystem = FTBGetSubsystemHelper::GetTBSubsystem(LaunchParams.Owner))
	{
		FTBProjectileSimData simData = FTBProjectileSimData(Projectile, ProjectilePhysicalProperties, DebugType);
		simData.SetId(Id);
		TBSubsystem->AddAndFireProjectileWithCallbacksAndUpdate(simData, LaunchParams, OnProjectileComplete, OnProjectileHit, OnProjectileExitHit, OnProjectileInjure, OnUpdate, DebugType);
	}
	return FTBProjectileId::None;
}

TArray<FTBProjectileId> UTerminalBallisticsStatics::AddAndFireProjectiles(TArray<FTBProjectile> Projectiles, const FPhysMatProperties& ProjectilePhysicalProperties, TArray<FTBLaunchParams> LaunchParams, TArray<FTBProjectileId>& Ids, int32 DebugType)
{
	...
	if (Projectiles.Num() == LaunchParams.Num() && Projectiles.Num() > 0)
	{
		...
		if (UTerminalBallisticsSubsystem* TBSubsystem = FTBGetSubsystemHelper::GetTBSubsystem(LaunchParams[0].Owner))
		{
			...
			ParallelFor(Projectiles.Num(), [&](int32 i) {
				...
				FTBProjectileSimData simData = FTBProjectileSimData(Projectile, ProjectilePhysicalProperties, DebugType);
				simData.SetId(Ids[i]);
				const FTBProjectileId Id = TBSubsystem->AddAndFireProjectile(simData, LaunchParams[i], DebugType);
				Out[i] = Id;
...
TArray<FTBProjectileId> UTerminalBallisticsStatics::AddAndFireProjectilesWithCallbacks(TArray<FTBProjectile> Projectiles, const FPhysMatProperties& ProjectilePhysicalProperties, TArray<FTBLaunchParams> LaunchParams, TArray<FTBProjectileId>& Ids, FBPOnProjectileComplete OnProjectileComplete, FBPOnProjectileHit OnProjectileHit, FBPOnProjectileExitHit OnProjectileExitHit, FBPOnProjectileInjure OnProjectileInjure, int32 DebugType)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UTerminalBallisticsStatics::AddAndFireProjectiles);
	if (Projectiles.Num() == LaunchParams.Num() && Projectiles.Num() > 0)
	{
		...
		if (UTerminalBallisticsSubsystem* TBSubsystem = FTBGetSubsystemHelper::GetTBSubsystem(LaunchParams[0].Owner))
		{
			...
			ParallelFor(Projectiles.Num(), [&](int32 i) {
				...
				FTBProjectileSimData simData = FTBProjectileSimData(Projectile, ProjectilePhysicalProperties, DebugType);
				simData.SetId(Ids[i]);
				const FTBProjectileId Id = TBSubsystem->AddAndFireProjectileWithCallbacks(simData, LaunchParams[i], OnProjectileComplete, OnProjectileHit, OnProjectileExitHit, OnProjectileInjure, DebugType);
				Out[i] = Id;
			}, EParallelForFlags::Unbalanced);
			Out.Shrink();
			return Out;
...
TArray<FTBProjectileId> UTerminalBallisticsStatics::AddAndFireProjectilesWithCallbacksAndUpdate(TArray<FTBProjectile> Projectiles, const FPhysMatProperties& ProjectilePhysicalProperties, TArray<FTBLaunchParams> LaunchParams, TArray<FTBProjectileId>& Ids, FBPOnProjectileComplete OnProjectileComplete, FBPOnProjectileHit OnProjectileHit, FBPOnProjectileExitHit OnProjectileExitHit, FBPOnProjectileInjure OnProjectileInjure, FBPOnProjectileUpdate OnUpdate, int32 DebugType)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UTerminalBallisticsStatics::AddAndFireProjectiles);
	if (Projectiles.Num() == LaunchParams.Num() && Projectiles.Num() > 0)
	{
		...
		if (UTerminalBallisticsSubsystem* TBSubsystem = FTBGetSubsystemHelper::GetTBSubsystem(LaunchParams[0].Owner))
		{
			...
			ParallelFor(Projectiles.Num(), [&](int32 i) {
				...
				FTBProjectileSimData simData = FTBProjectileSimData(Projectile, ProjectilePhysicalProperties, DebugType);
				simData.SetId(Ids[i]);
				const FTBProjectileId Id = TBSubsystem->AddAndFireProjectileWithCallbacksAndUpdate(simData, LaunchParams[i], OnProjectileComplete, OnProjectileHit, OnProjectileExitHit, OnProjectileInjure, OnUpdate, DebugType);
				Out[i] = Id;
			}, EParallelForFlags::Unbalanced);
			Out.Shrink();
			return Out;
...
FTBProjectileId UTerminalBallisticsStatics::AddAndFireTestBullet(TSoftObjectPtr<class UBulletDataAsset> Bullet, FTBLaunchParams& LaunchParams, TArray<FPredictProjectilePathPointData>& Results, FLatentActionInfo LatentInfo, const FTBProjectileId& Id)
{
	if (LaunchParams.Owner)
	{
		if (UWorld* World = LaunchParams.Owner->GetWorld())
		{
			if (UTerminalBallisticsSubsystem* TBSubsystem = FTBGetSubsystemHelper::GetTBSubsystem(LaunchParams.Owner))
			{
				...
				const FTBProjectileId AssignedId = TBSubsystem->AddAndFireBullet(SimData, LaunchParams);
				World->GetLatentActionManager().AddNewAction(LaunchParams.Owner, LatentInfo.UUID, LatentAction);
				return AssignedId;
			}
		}
	}
...
TArray<FPredictProjectilePathPointData> UTerminalBallisticsStatics::AddAndFireTestBullet(TSoftObjectPtr<UBulletDataAsset> Bullet, FTBLaunchParams& LaunchParams, const FTBProjectileId& Id)
{
	if (LaunchParams.Owner)
	{
		if (UWorld* World = LaunchParams.Owner->GetWorld())
		{
			if (UTerminalBallisticsSubsystem* TBSubsystem = FTBGetSubsystemHelper::GetTBSubsystem(LaunchParams.Owner))
			{
				...
				const auto& Results = TBSubsystem->FireBulletSynchronous(&SimData, LaunchParams, FTBBulletTaskCallbacks());

				const FTBProjectileId ActualId = SimData.GetId();

				return Results.PathData;
			}
		}
	}
	return {};
...
DEFINE_FUNCTION(UTerminalBallisticsStatics::execMakePayload)
{
	P_GET_OBJECT(UObject, Z_Param_Object);
	P_GET_UBOOL_REF(Z_Param_Success);
	P_FINISH;
	P_NATIVE_BEGIN;
	*(UObject**)Z_Param__Result = Z_Param_Object;
	Z_Param_Success = IsValid(Cast<UObject>(Z_Param_Object));
	P_NATIVE_END;
}
