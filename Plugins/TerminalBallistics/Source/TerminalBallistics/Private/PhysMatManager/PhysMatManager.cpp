...
bool UTBPhysMatManager::RemoveEntry(const FGameplayTag& Tag)
{
	return PhysMatMap.Remove(Tag) > 0;
}
...
FPhysMatProperties UTBPhysMatManager::GetFromHitResult(const FHitResult& HitResult, bool& Success) const
{
	...
	if (HitResult.GetComponent()->Implements<UTBDynamicMaterialPropertyManager>())
	{
		return ITBDynamicMaterialPropertyManager::Execute_GetMaterialProperties(HitResult.GetComponent(), HitResult);
	}
	else if (HitResult.GetActor()->Implements<UTBDynamicMaterialPropertyManager>())
	{
		return ITBDynamicMaterialPropertyManager::Execute_GetMaterialProperties(HitResult.GetActor(), HitResult);
	}
...
bool UTBPhysMatManager::ImplementsDynamicMaterialPropertyManager(UObject* Object) const
{
	return TB::ImplementsDynamicMaterialPropertyManager(Object);
}
...
static constexpr double OldLeadKic = 0.18;
static constexpr double OldPaperKic = 0.1;
void UTBPhysMatManager::BuildMap()
{
	...
	TBConfig->bIsUpdatingPhysMatMapInternally = true;
	auto OutdatedLeadMapping = TBConfig->PhysMats.FindByPredicate([](const FTBPhysMatMapping& Mapping) { return Mapping.PhysMatProperties.MaterialName == "Lead"; });
	if (OutdatedLeadMapping)
	{
		if (FMath::IsNearlyEqual(OutdatedLeadMapping->PhysMatProperties.FractureToughness, OldLeadKic))
		{
			OutdatedLeadMapping->PhysMatProperties = TB::PhysMatConstants::DefaultPhysMats::Lead();
		}
	}
	auto OutdatedPaperMapping = TBConfig->PhysMats.FindByPredicate([](const FTBPhysMatMapping& Mapping) { return Mapping.PhysMatProperties.MaterialName == "Paper"; });
	if (OutdatedPaperMapping)
	{
		if (FMath::IsNearlyEqual(OutdatedPaperMapping->PhysMatProperties.FractureToughness, OldPaperKic))
		{
			OutdatedPaperMapping->PhysMatProperties = TB::PhysMatConstants::DefaultPhysMats::Paper();
		}
	}
	TBConfig->bIsUpdatingPhysMatMapInternally = false;

	for (auto& PhysMat : TBConfig->PhysMats)
	{
