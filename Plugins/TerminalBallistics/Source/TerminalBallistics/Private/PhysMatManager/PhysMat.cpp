...
FArchive& operator<<(FArchive& Ar, FTBSimplePhysMatProperties& PhysMatProperties)
{
	Ar << PhysMatProperties.PenetrationResistance;
	Ar << PhysMatProperties.PenetrationRequirement;
	Ar << PhysMatProperties.EnergyRequiredToPenetrate;
	Ar << PhysMatProperties.EnergyLossPerCm;
	return Ar;
}
...
bool FPhysMatProperties::ShouldUseSimpleProperties() const
{
	ETBPenetrationComplexity Complexity = bOverridePenetrationComplexity ? PenetrationComplexity : TB::Configuration::PenetrationComplexity;
	return static_cast<uint8>(Complexity & ETBPenetrationComplexity::Simplified) > 0U;
}
...
double FPhysMatProperties::EstimateK() const
{
	const double Term1_0 = 1.0 - PoissonsRatio * PoissonsRatio;
	const double Term1_1 = YoungsModulus * Term1_0;
	const double Term1 = FMath::Loge(Term1_1 * Term1_1) / 40.0;
	return (Term1 * Term1) / 3.0;
}
...
double FPhysMatProperties::CalculateStrainEnergy(const double Stress, const double Volume) const
{
	using namespace TB::UnitConversions;
	using namespace TB::MathUtils;
	const double YoungsModulusInBaseUnits = bIsInBaseUnits ? YoungsModulus : GPaToPa(YoungsModulus);
	return 0.5 * (Volume / YoungsModulusInBaseUnits) * sqr(Stress);
}

double FPhysMatProperties::GetCriticalEnergyReleaseRate(const bool bIsPlaneStress) const
{
	using TB::MathUtils::sqr;
	const double E = bIsPlaneStress ? YoungsModulus : YoungsModulus / (1 - sqr(PoissonsRatio));
	return sqr(FractureToughness) / E;
}
...
double FPhysMatProperties::CalculateStrainEnergyReleaseRate(const double ImpactVelocity, const double CrackLength) const
{
	...
	using namespace TB::MathUtils;
	const double G = sqr(K) / (E * UE_DOUBLE_PI * CrackLength);
	return G * pow(CalculateStrainRate(ImpactVelocity, CrackLength), StrainRateFactor);
}
...
double FPhysMatProperties::CalculateEnergyRequiredToPropagateFracture(const double ImpactVelocity, const double CrackArea, const double Depth) const
{
	...
	return G * pow(CalculateStrainRate(ImpactVelocity, Depth), StrainRateFactor) * CrackArea;
}

bool FPhysMatProperties::NetSerialize(FArchive& Ar, UPackageMap* Map, bool& bOutSuccess)
{
	TB_PACK_ARCHIVE_WITH_BITFIELDS_FIVE(Ar, bIsFluid, bIsParticulate, bUseCustomRicochetProperties, bIsInBaseUnits, bOverridePenetrationComplexity);
	Ar << MaterialName;
	Ar << SurfaceType;
	Ar << Density;
	...
	Ar << Viscosity;
	Ar << StrainRateFactor;
	Ar << PenetrationResistanceMultiplier;
	Ar << RicochetProperties;
	Ar << SimpleProperties;
	Ar << PenetrationComplexity;
...
namespace TB
{
	namespace PhysMatConstants
	{
		...
		namespace DefaultPhysMats
		{
			...
			{
				return FPhysMatProperties(FGameplayTag::RequestGameplayTag(TagName_Lead), SurfaceType20, Names::PhysMatName_Lead, 11.3, 10, 17, 14, 12, 4.5, 0.44, 16, 0.f);
			}
			...
			{
				return FPhysMatProperties(FGameplayTag::RequestGameplayTag(TagName_Paper), SurfaceType22, Names::PhysMatName_Paper, 0.6, 0.5, 30, -1, 30, 12, 0.43, 3, 0.f);
			}
			...
			{
				return GetDefaults().FindByPredicate([Name](const FPhysMatProperties& PhysMat) { return PhysMat.MaterialName == Name; });
			}
