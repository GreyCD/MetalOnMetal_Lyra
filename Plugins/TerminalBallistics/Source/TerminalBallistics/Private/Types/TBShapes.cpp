...
double FTBOgive::GetFrontalCSA(const bool bForceRecalculate) const
{
	...
	{
		FrontalCSA = TB::MathUtils::sqr(Radius / 100.0) * UE_DOUBLE_PI;
	}
	return FrontalCSA;
}
...
double FTBOgive::GetShapeVolume(const bool bForceRecalculate) const
{
	if (Volume <= 0 || bForceRecalculate)
	{
		...
		while (x < Length)
		{
			const double radius = GetOgiveRadius(x);
			const double prevRadius = GetOgiveRadius(x - dx);
			Volume += 1/1000000.0 * OneThirdPi * dx * (sqr(prevRadius) + radius * prevRadius + sqr(radius));
			x += dx;
		}
	}
	return Volume;
