...
const TAutoConsoleVariable<int32> CVarWorkerThreadStackSize(
	TEXT("tb.Thread.WorkerThread.StackSize"),
	256,
	TEXT("Amount of memory to available to each worker thread. (KiB)"),
	ECVF_Cheat
);

const TAutoConsoleVariable<bool> CVarWorkerThreadShowStats(
	TEXT("tb.Thread.WorkerThread.ShowStats"),
	false,
	TEXT("Displays average tick duration (ms) for active projectile worker threads"),
	ECVF_Cheat
);

const TAutoConsoleVariable<bool> CVarWorkerThreadShowExhaustion(
	TEXT("tb.Thread.WorkerThread.LogExhaustion"),
	false,
	TEXT("Logs instances of thread exhaustion, where the tick rate falls below the nominal value"),
	ECVF_Cheat
);

#pragma region FTBProjectileThread
static TB::SimTasks::FTBProjectileThreadPoolParams GetThreadPoolParams(const FTimespan& TickRate, const uint32 ThreadStackSize)
{
	...
	WorkerThreadParams.Emplace(FTBProjectileWorkerParams::Projectiles);
	WorkerThreadParams.Emplace(FTBProjectileWorkerParams::Projectiles);
	FTBProjectileThreadPoolParams ThreadPoolParams = FTBProjectileThreadPoolParams(TickRate, WorkerThreadParams, ThreadStackSize);
...
void FTBProjectileThreadPool::RequestThreadTransfer(TSharedPtr<ProjectileTask> Task, WorkerThreadPtr CurrentThread)
{
	if (WorkerThreadPtr NewThread = FindOrAddThreadForTask(CurrentThread->GetTaskAffinity(), CurrentThread))
	{
		NewThread->AddTask(Task);
	}
}
...
void FTBProjectileThreadPool::Shrink()
{
	...
	FScopeLock RemoveLock(&TaskRemovalMutex);
...
WorkerThreadArray FTBProjectileThreadPool::GetNonFullThreads(WorkerThreadPtr ToIgnore)
{
	LLM_SCOPE_BYTAG(FTBProjectileThreadPool);
	auto ToReturn = Threads.FilterByPredicate([&](const WorkerThreadPtr Thread) { return Thread && Thread->GetNum() != MaxTasksPerThread; });
	if (ToIgnore)
	{
		ToReturn.RemoveSingle(ToIgnore);
	}
	ToReturn.Shrink();
	return ToReturn;
}

WorkerThreadArray FTBProjectileThreadPool::GetThreadsWithAffinity(ThreadTaskAffinity Affinity, WorkerThreadPtr ToIgnore)
{
	LLM_SCOPE_BYTAG(FTBProjectileThreadPool);
...
WorkerThreadPtr FTBProjectileThreadPool::FindOrAddThreadForTask(ThreadTaskAffinity Affinity, WorkerThreadPtr ToIgnore)
{
	LLM_SCOPE_BYTAG(FTBProjectileThreadPool);
	...
	const auto AffinityPred = [Affinity, ToIgnore](const WorkerThreadPtr Thread) { return Thread && Thread != ToIgnore && Thread->GetTaskAffinity() == Affinity; };
	const auto CanAddPred = [Affinity, ToIgnore](const WorkerThreadPtr Thread) { return Thread && Thread != ToIgnore && Thread->CanAcceptTask(Affinity); };
	auto RemoveIgnoredThread = [ToIgnore](WorkerThreadArray& ThreadsLocal) { if (ToIgnore) { ThreadsLocal.RemoveSingle(ToIgnore); } };
	...
	WorkerThreadArray NotFull = GetNonFullThreads(ToIgnore);
	NotFull.Shrink();
	{
		if (NotFull.Num() == 0)
		{
			...
			else
			{
				...
				if (!FoundThread)
				{
					...
					{ // Can't add a new thread, and there are none with our preferred affinity
						WorkerThreadArray ThreadsWithNoAffinity = GetThreadsWithAffinity(ThreadTaskAffinity::NONE, ToIgnore);
	...
	/* See if there are any threads that will accept the task */
	{
		...
		RemoveIgnoredThread(CanAcceptTask);
	...
	WorkerThreadArray PreferredThreads = GetThreadsWithAffinity(Affinity, ToIgnore);
	WorkerThreadArray ThreadsWithNoAffinity = GetThreadsWithAffinity(ThreadTaskAffinity::NONE, ToIgnore);
...
void FTBProjectileWorkerThread::TickTasks()
{
	...
	if (WasPaused) // We were previously paused, if we continue now then it would be as if the pause was just a long tick and projectiles would simulate as if we didn't pause.
	{
		...
		WasPaused = false;
		TickExit = TickEntry;
		std::lock_guard<std::mutex> Lock(TaskIteratorMutex);
		const int32 Num = SimTasks.Num();
		for (int i = 0; i < Num; i++)
		{
			TaskPtr& Task = SimTasks[i];
			if (!Task)
			{
				continue;
			}
	...
	{
		const double NominalTickRate = GetNominalTickRate();
		const double TimeSpentInUpdate = abs(duration_cast<TB::SimTasks::double_seconds>(TickExit - TickEntry)).count();

		CurrentTickRate = (CurrentTickRate + TimeSpentInUpdate) / 2.0;

		if (TimeSpentInUpdate > NominalTickRate && CVarWorkerThreadShowExhaustion.GetValueOnAnyThread())
		{
	...
	if(CVarWorkerThreadShowStats.GetValueOnAnyThread() && GEngine)
	{
		int val = 0;
		{
			const int len = ThreadName.Len();
			for (int i = 0; i < len; i++)
			{
				val += ThreadName[i];
			}
		}
		FString msg = FString::Printf(TEXT("%s: %f"), *GetThreadName(), GetCurrentTickRateMs());
		GEngine->AddOnScreenDebugMessage(val, 0.125f, FColor::Red, msg, false);
	}

	InTickLoop = false;
}

void FTBProjectileWorkerThread::RecieveTask(TaskPtr NewTask)
{
	LLM_SCOPE_BYTAG(FTBProjectileWorkerThread);
...
void FTBProjectileWorkerThread::AddTask(TaskPtr NewTask, const bool ForceTick)
{
	LLM_SCOPE_BYTAG(FTBProjectileWorkerThread);
	if(ForceTick)
	{
