...
#pragma endregion
	...
	static FORCEINLINE FQuat IntegrateAngularVelocity(const FQuat& R0, const FVector& AngularVelocity, const double dt)
	{
	...
	*/
	static FORCEINLINE double GetSaturationVaporPressureOfWater(const double T)
	{
		...
		const double logT = log10(Tc / 273.15);
		const double C = pow(10, C2 - D3 / Tc);
		const double D = D1 * pow(10, D2 - D3 / Tc);
		const double E = exp(B + A * (1.0 - C + D * (1.0 - C) * (1.0 - C)));
	...
	*/
	static FORCEINLINE double GetCriticalRicochetAngleForFluid(const double FluidDensity, const double ObjectDensity)
	{
		return 18.0 / sqrt(FluidDensity / ObjectDensity);
	}

	static FORCEINLINE double GetProjectileVelocityInFluid(const double InitialVelocity, const double DragForce, const double Depth)
	{
		return InitialVelocity * exp(-DragForce * Depth);
	}

	static FORCEINLINE double GetFroudeNumber(const double Radius, const double Velocity, const double Gravity)
	{
		return Velocity / sqrt(Radius * Gravity);
	}
	...
	*/
	static FORCEINLINE double CalculateDynamicPressure(double Density, double Speed)
	{
	...
	*/
	static FORCEINLINE double CalculateKineticEnergy(const double Mass, const double Speed)
	{
	...
	* @return				Speed (m/s)
	*/
	static FORCEINLINE double CalculateVelocityFromKineticEnergy(const double KineticEnergy, const double Mass)
	{
		UE_ASSUME(Mass > 0);
		const double NewV = sqrt((2.0 * abs(KineticEnergy)) / Mass);
		return copysign(NewV, KineticEnergy); // Deal with the case where KineticEnergy is negative
	}
	...
	static FORCEINLINE double CalculateElasticDeformationEnergy(const double DeformedLength, const double OriginalLength, const double YoungsModulus)
	{
		const double DeltaLength = OriginalLength - DeformedLength;
		const double Strain = DeltaLength / OriginalLength;
		return 0.5 * YoungsModulus * pow(Strain, 2.0);
	}
	static FORCEINLINE double CalculateElasticDeformationEnergy(const double Displacement, const double YoungsModulus)
	{
		return 0.5 * YoungsModulus * pow(Displacement, 2.0);
	}
	...
	* @return				Pressure (Pa)
	*/
	static FORCEINLINE double CalculateAirPressureAtAltitude(const double g, const double h, const double T, const double P0 = TB::Constants::ReferencePressure_SeaLevel)
	{
	...
	*/
	static FORCEINLINE double CalculateAirDensity(const double p, const double T, const double Rh)
	{
	...
	* @param ReferenceHumidity	Humidity at sea level
	*/
	static FORCEINLINE double CalculateHumidity(const double Temperature, double Pressure, double Density, double ReferenceHumidity)
	{
	...
	* @param P			Absolute Pressure (Pa)
	*/
	static FORCEINLINE double CalculateAirViscosity(const double T, const double P)
	{
		...
		using namespace TB::Constants;
		double mu = DynamicViscosity_Air * (T0 + C) / (T1 + C) * pow(T1 / T0, 1.5);
	...
	* @return					Stress (Pa)
	*/
	static FORCEINLINE double CalculateImpactStress(const double KineticEnergy, const double ImpactArea, const double ImpactAngle, const double ObjectThickness, const double ObjectE, const double ProjectileE, const double ObjectKic)
	{
		...
		const double Denominator = ((ObjectE + ProjectileE) * ObjectThickness * sqrt(DOUBLE_PI * ImpactArea * CosAngle) / 2);
	...
	namespace Drag
	{
		namespace DragModels
		{
			...
			namespace Private
			{
				...
				protected:
					...
					static FORCEINLINE long double GetSubsonic(const double Speed)
					{
						const long double exp_1 = a1 * pow(Speed, b1);
						const long double exp_2 = pow(Speed, c1);
						const long double exp_3 = pow(d1, Speed);
					...
					static FORCEINLINE long double GetSupersonic(const double Speed)
					{
						const long double denominator = b2 + pow(Speed, d2);
						return c2 + (a2 / denominator) - n2 * pow(Speed, -4.0L);
					}

				public:
					static FORCEINLINE long double GetDragCoefficient(const double Speed)
					{
						const double asMach = abs(Speed) / Constants::Mach1;
				...
				protected:
					...
					static FORCEINLINE long double GetSupersonic(const double Speed)
					{
						const long double exp_1 = pow(Speed, n1);
						const long double exp_2 = pow(b1, Speed);
					...
					static FORCEINLINE long double GetSubsonic(const double Speed)
					{
						const long double exp_1 = a2 * pow(Speed, n2);
						const long double exp_2 = b2 * pow(c2, Speed);
				...
				protected:
					...
					static FORCEINLINE long double GetRegion1(const long double Re)
					{
						const auto exp_1 = a1 * pow(Re, b1);
						const auto exp_2 = pow(d1, Re);
						const auto exp_3 = f1 * pow(g1, 2.0L * Re);
					...
					static FORCEINLINE long double GetRegion2(const long double Re)
					{
						const auto exp_1 = a2 * pow(Re, b2);
						const auto exp_2 = d2 * pow(f2, Re);
					...
					static FORCEINLINE long double GetRegion3(const long double Re)
					{
						const auto exp_1 = a3 * pow(Re, b3);
						const auto exp_2 = d3 * pow(f3, Re);
						const auto exp_3 = g3 * pow(Re, h3);
						return exp_1 + exp_2 + exp_3 + c3;
					}

				public:
					static FORCEINLINE long double GetDragCoefficient(const long double ReynoldsNumber)
					{
						const long double CorrectionFactor = 1.1 * pow(ReynoldsNumber, -ScalingFactor / 2.0L);
						const long double Re = abs(pow(ReynoldsNumber, ScalingFactor));
						if (Re < Region2Start)
						{
		...
		* @param p			Fluid density of the medium the object is passing through. Defaults to air.
		*/
		static FORCEINLINE double CalculateDragForce(const double cd, const double csa, const double speed, const double windSpeed = 0, double p = -1)
		{
