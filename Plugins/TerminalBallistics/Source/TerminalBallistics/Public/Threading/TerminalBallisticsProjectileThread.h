...
public:
	...
	explicit FTBProjectileThreadPool(const FTBProjectileThreadPoolParams& Params)
	...
	void RequestThreadTransfer(TSharedPtr<ProjectileTask> Task, WorkerThreadPtr CurrentThread);

	FCriticalSection TaskRemovalMutex;
	FCriticalSection TaskAdditionMutex;

	TConstArrayView<WorkerThreadPtr> GetThreads() const { return Threads; }

protected:
	inline bool CanAddThread() const
	{
		return !(CapNumThreads && Threads.Num() >= MaxThreads);
	}

	TArray<WorkerThreadPtr> GetNonFullThreads(WorkerThreadPtr ToIgnore = nullptr);
	TArray<WorkerThreadPtr> GetThreadsWithAffinity(ThreadTaskAffinity Affinity = ThreadTaskAffinity::NONE, WorkerThreadPtr ToIgnore = nullptr);
	WorkerThreadPtr FindOrAddThreadForTask(ThreadTaskAffinity Affinity = ThreadTaskAffinity::NONE, WorkerThreadPtr ToIgnore = nullptr);
...
{
public:
	FTBProjectileThread(const FTimespan& TickRate, const TCHAR* ThreadName, class UTerminalBallisticsSubsystem* Controller, class UTBProjectileThreadQueue* DataQueue);
...
protected:
	...
	TSharedPtr<FTBProjectileThreadPool> ThreadPool = nullptr;
};
...
public:
	FTBProjectileWorkerThread(FTBProjectileThreadPool* OwningPool, const FTimespan& TickRate, const TCHAR* ThreadName, const uint32 StackSize = 128 * 1024, const int32 InMaxTasks = 100, const TaskOverflowBehavior OverflowBehavior = TaskOverflowBehavior::CreateNew, const ThreadTaskAffinity TaskAffinity = ThreadTaskAffinity::NONE, const bool AffinityIsStrong = false)
		...
		, HasStopped(false)
		, TickRate(TickRate)
		, CurrentTickRate(GetNominalTickRate())
	...
	void RecieveTask(TaskPtr NewTask);
	...
	inline double GetNominalTickRate() const { return TickRate.GetTotalSeconds(); }

	inline double GetCurrentTickRate() const { return CurrentTickRate; }
	inline double GetCurrentTickRateMs() const { return CurrentTickRate * 1e3; }

	FString GetThreadName() const { return ThreadName; }
	...
	void AddTask(TaskPtr NewTask, const bool ForceTick = false);
...
protected:
	...
	mutable std::mutex RecieveTaskMutex;
	mutable std::mutex TaskIteratorMutex; // Do not remove tasks while this lock is aquired
...
inline void FTBProjectileThreadPool::AddNewTask(ThreadTaskAffinity Affinity, FTBProjectileThread* ProjectileThread, typename TaskType::SimDataType&& SimData)
{
	...
	if (ThreadForTask)
	{
		...
		if (NumBeforeAdd >= ThreadForTask->GetMax())
		{
			...
			{
				TB_LOG(Verbose, TEXT("Warning: added task to thread with %i tasks."), NumBeforeAdd);
			}
		}
