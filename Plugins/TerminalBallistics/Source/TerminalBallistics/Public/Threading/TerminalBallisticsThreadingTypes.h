...
#include "GraphTasks/TBTaskGuard.h"
#include "Misc/TBCollisionPresets.h"
#include "Misc/TBForward.h"
...
namespace TB::SimTasks
{
	...
	class FActivateTracerTask : public FTaskWithCheckValue, public FPendingTask
	{
	public:
		using GraphTask = TGraphTask<FActivateTracerTask>;

		FActivateTracerTask(UNiagaraComponent* Tracer, bool* HasBeenActivated, FPendingTaskSynch* SynchObject = nullptr, bool* CheckValue = nullptr)
			: FTaskWithCheckValue(CheckValue)
			, FPendingTask(SynchObject)
		...
		FORCEINLINE TStatId GetStatId() const
		{
			RETURN_QUICK_DECLARE_CYCLE_STAT(FActivateTracerTask, STATGROUP_TaskGraphTasks);
		}
	...
	public:
		...
		TProjectileSimulationTask(FTBBaseProjectileThread* Controller, TypeOfSimData& InSimData)
			...
			, bIsDone(false)
			, HasExited(false)
			, bHasBroadcastCompletion(false)
			, bFirstUpdate(true)
			, bRetryHitConsumption(false)
			, bUseDragVector(true)
			, bIsSynchronous(false)
			, bIsOnGameThread(IsInGameThread())
			, bNotifyImpacts(true)
		...
		inline void UpdateDrag(const FVector& Location, const FVector& Velocity, const float FluidDensity = 0.0f)
		{
			...
			if (bUseDragVector)
			{
				CurrentDragVector = TerminalBallisticsSubsystem->CalculateProjectileDrag(VelocityMS, GravityZ, Location, Velocity.ToOrientationQuat(), ProjectileProperties, DragCalculationType, bIsInFluid ? FluidDensity : -1, !bIsInFluid);
			}
			else
			{
		...
		ResultType GetResults() const
		{
			return Results;
		}

	protected:
		virtual typename ProjectilePropertyStruct::ProjectileStruct& GetProjectile() = 0;
	public:
		typename ProjectilePropertyStruct::ProjectileStruct GetProjectile() const
		{
			auto mutableThis = const_cast<std::remove_const_t<decltype(this)>>(this);
			return mutableThis->GetProjectile();
		}
	...
	protected:
		...
		[[nodiscard]] FPendingTaskGuard RequestTaskGuard();

		virtual void CallUpdateFunc();
		virtual void BroadcastUpdateDelegate();

		void BroadcastHitDelegate(ImpactStruct ImpactParams);
		void BroadcastExitHitDelegate(ImpactStruct ImpactParams);
		void BroadcastInjureDelegate(ImpactStruct ImpactParams, FTBProjectileInjuryParams InjuryParams);
		...
		*/
		...
		bool bHasBroadcastCompletion : 1;
		...
		bool bUseDragVector : 1;

		bool bIsSynchronous : 1;

		bool bIsOnGameThread : 1;
		...
		double OwnerIgnoreDistanceCm = 10.0;

		double TracerActivationDistanceCm = 50.0;
...
#pragma region Inline Functions
	...
	template<typename TBSubsystem, typename TypeOfSimData, typename ProjectilePropertyStruct, typename DelegateStructType>
	inline FPendingTaskGuard TProjectileSimulationTask<TBSubsystem, TypeOfSimData, ProjectilePropertyStruct, DelegateStructType>::RequestTaskGuard()
	{
		return FPendingTaskGuard(&PendingTaskSynch);
	}
	...
	void TProjectileSimulationTask<TBSubsystem, ImpactStruct, ProjectilePropertyStruct, DelegateStructType>::CallUpdateFunc()
	{
		...
		if (CallOnUpdate)
		{
			if (CallOnUpdateFuncFromGameThread && !bIsSynchronous)
			{
		...
		if (bDebugDraw)
		{
			...
			GameThreadTask([WorldLocal = World, PrevData = PreviousFlightData, CurrData = GetCurrentFlightData(), InitialVel = Results.InitialVelocity.Size(), Vel = PreviousFlightData.Velocity.Size(), Guard]()
				{
					FPendingTaskGuard TaskGuard(Guard);
					if (WorldLocal)
					{
...
#pragma region Delegates
	...
	{
		if (bIsShuttingDown || bHasBroadcastCompletion)
		{
			return;
		}
		bHasBroadcastCompletion = true;
		if(CompletionDelegate.IsBound())
		{
			InitiateTask<FCompletionTask>(bIsSynchronous, CompletionDelegate, Results, &PendingTaskSynch, &bIsShuttingDown);
		}
	}
	...
	void TProjectileSimulationTask<TBSubsystem, ImpactStruct, DelegateStructType, OnUpdateParamType>::BroadcastCompletionDelegateBasic()
	{
		...
		{
			InitiateTask<FCompletionTaskBasic>(bIsSynchronous, Delegates.OnComplete, Id, Results.PathData, &PendingTaskSynch, &bIsShuttingDown);
		}
	}

	template<typename TBSubsystem, typename ImpactStruct, typename DelegateStructType, typename OnUpdateParamType>
	void TProjectileSimulationTask<TBSubsystem, ImpactStruct, DelegateStructType, OnUpdateParamType>::BroadcastHitDelegate(ImpactStruct ImpactParams)
	{
		if (bIsShuttingDown)
		{
			return;
		...
		if (bNotifyImpacts && ImpactParams.IsValid())
		{
			...
			{
				InitiateTask<FHitTask>(bIsSynchronous, Delegates.OnHit, ImpactParams, &PendingTaskSynch, &bIsShuttingDown);
			}
			if (BoundFunctions.HitFunction)
			{
				auto Func = std::bind(BoundFunctions.HitFunction, ImpactParams);
				InitiateTask<FLambdaTask>(bIsSynchronous, MoveTemp(Func), &PendingTaskSynch, &bIsShuttingDown);
			}
		}
	}

	template<typename TBSubsystem, typename ImpactStruct, typename DelegateStructType, typename OnUpdateParamType>
	void TProjectileSimulationTask<TBSubsystem, ImpactStruct, DelegateStructType, OnUpdateParamType>::BroadcastExitHitDelegate(ImpactStruct ImpactParams)
	{
		if (bIsShuttingDown)
		{
			return;
		...
		if (bNotifyImpacts && ImpactParams.IsValid())
		{
			...
			{
				InitiateTask<FExitHitTask>(bIsSynchronous, Delegates.OnExitHit, ImpactParams, &PendingTaskSynch, &bIsShuttingDown);
			}
	...
	template<typename TBSubsystem, typename ImpactStruct, typename DelegateStructType, typename OnUpdateParamType>
	void TProjectileSimulationTask<TBSubsystem, ImpactStruct, DelegateStructType, OnUpdateParamType>::BroadcastInjureDelegate(ImpactStruct ImpactParams, FTBProjectileInjuryParams InjuryParams)
	{
		if (bIsShuttingDown)
		{
			return;
		...
		if (bNotifyImpacts && ImpactParams.IsValid() && InjuryParams.IsValid())
		{
			...
			{
				InitiateTask<FInjureTask>(bIsSynchronous, Delegates.OnInjure, ImpactParams, InjuryParams, &PendingTaskSynch, &bIsShuttingDown);
			}
	...
	void TProjectileSimulationTask<TBSubsystem, TypeOfSimData, ProjectilePropertyStruct, DelegateStructType>::BroadcastUpdateDelegate()
	{
		...
		{
			InitiateTask<FOnUpdateTask>(bIsSynchronous, Delegates.OnUpdate, GetCurrentFlightData(), &PendingTaskSynch, &bIsShuttingDown);
		}
	}
#pragma endregion
	...
	void TProjectileSimulationTask<TBSubsystem, TypeOfSimData, ProjectilePropertyStruct, DelegateStructType>::PopulateSimData(TypeOfSimData& SimData)
	{
		...
		OwnerIgnoreDistanceCm = SimData.OwnerIgnoreDistance * 100.0;
		TracerActivationDistanceCm = SimData.TracerActivationDistance * 100.0;
		...
		bIsSynchronous = SimData.bIsSynchronous || SimData.IsTestTask;

		if (Owner)
		{
			bIgnoreOwner = !QueryParams.GetIgnoredActors().Contains(Owner->GetUniqueID());
		}
		else
		{
			bIgnoreOwner = false;
		...
		if (Tracer)
		{
			/* Prepare the "Kill Tracer" graph task and get it ready to launch when prerequisites are met. */
	...
	inline typename TProjectileSimulationTask<TBSubsystem, TypeOfSimData, ProjectilePropertyStruct, DelegateStructType>::FAttemptHit TProjectileSimulationTask<TBSubsystem, TypeOfSimData, ProjectilePropertyStruct, DelegateStructType>::AttemptHitCheck(const double PreviousTime, const double dt, const bool bBroadcastUpdate)
	{
		...
		const double DistanceTravelled = FVector::Dist(StartLocation, GetLocation());

		if (bIgnoreOwner)
		{
			if (DistanceTravelled > OwnerIgnoreDistanceCm)
			{
				bIgnoreOwner = false;
			}
			else if (const AActor* OwnerPtr = Owner.Get())
			{
		...
		if (!bIsTracerActive && DistanceTravelled >= TracerActivationDistanceCm)
		{
			if (Tracer)
			{
				TGraphTask<FActivateTracerTask>::CreateTask().ConstructAndDispatchWhenReady(Tracer, &bIsTracerActive, &PendingTaskSynch, &bIsShuttingDown);
				UpdateTracer();
			}
		}
		...
		if (PreviousHitResult.GetComponent() && TB::DoesShapeOverlap(PreviousHitResult, GetLocation(), Sphere, FQuat::Identity, PreviousShapeIndex))
		{
			if (!bIsInFluid)
			{
	...
	void TProjectileSimulationTask<TBSubsystem, TypeOfSimData, ProjectilePropertyStruct, DelegateStructType>::SetupSimulationVariables(const FPredictProjectilePathParams& PredictParams)
	{
		...
		MaxPenetrations = TB::Configuration::MaxPenetrations;

		const bool SimFrequencyIsValid = PredictParams.SimFrequency > KINDA_SMALL_NUMBER;
		if (World && SimFrequencyIsValid)
		{
			bUseDragVector = DragCalculationType == ETBDragComplexity::WIND3D || DragCalculationType == ETBDragComplexity::COMPLEX;
			...
			QueryParams = FCollisionQueryParams(SCENE_QUERY_STAT(TProjectileSimulationTask), PredictParams.bTraceComplex);
	...
	{
		const double dt = 1.0 / TB::Configuration::ThreadTickFrequency;
		while (SimTimeIsValid() && Iterations < MaxIterations && Penetrations < MaxPenetrations && !HasExited)
		{
			Iterations++;
			const double ActualDt = std::min(MaxSimTime - CurrentTime, dt);
			Update(ActualDt);
		}
	...
	void TProjectileSimulationTask<TBSubsystem, TypeOfSimData, ProjectilePropertyStruct, DelegateStructType>::ConsumeHits()
	{
		...
		if (UE::IsSavingPackage(nullptr) || IsGarbageCollecting())
		{
			/* Trying to continue further could result in a crash, since GC will interfere with any attempts to determine what we hit. */
		...
		if (!exitFound)
		{
			const ImpactStruct ImpactParams = ImpactStruct(LatestHitResult, GetProjectile(), GetVelocity(), true, SurfaceProperties.SurfaceType, StartLocation, Owner, false, Id, GetVelocity().Size());
			BroadcastHitDelegate(ImpactParams);
			Results.Add(ImpactParams);
			TB::DispatchMaterialDamageEvent(false, GetKineticEnergy(), LatestHitResult);
		...
		if (SurfaceProperties.bIsFluid)
		{
			...
			{
				const double PreviousEnergy = GetKineticEnergy();
				VelocityAtFluidEntry = GetVelocity() / 100.0;
				...
				const ImpactStruct ImpactParams = ImpactStruct(LatestHitResult, GetProjectile(), VelocityAtFluidEntry, true, SurfaceProperties.SurfaceType, StartLocation, Owner, bRicochet, Id, VelocityAtFluidEntry.Size() - NewVelocity.Size(), NewVelocity);
				BroadcastHitDelegate(ImpactParams);
				TB::DispatchMaterialDamageEvent(false, abs(PreviousEnergy - GetKineticEnergy()), LatestHitResult);
		...
		for (const FExitResult& ExitResult : ExitResults)
		{
			...
			PreviousShapeIndex = ExitResult.ShapeIndex;
			if(ExitCode != ConsumeHitReturnCode::Ignore)
			{
				TB::DispatchMaterialDamageEvent(ExitCode == ConsumeHitReturnCode::Penetration, abs(PreviousEnergy - GetKineticEnergy()), ExitResult.HitResult);
			}
			...
			else if (ExitCode == ConsumeHitReturnCode::Penetration)
			{
				Penetrations++;
			}
	...
	void TProjectileSimulationTask<TBSubsystem, TypeOfSimData, ProjectilePropertyStruct, DelegateStructType>::Update(const double dt)
	{
		...
		if (bIsShuttingDown || bIsDone || HasExited)
		{
			return;
		}
