...
public:
	...
	UPROPERTY(BlueprintReadWrite, EditAnywhere, Category = "Bullet", meta = (EditCondition = "ProjectileCount > 1", EditConditionHides))
	...
	FTBBullet(const FTBBulletPhysicalProperties& Properties, const FPhysMatProperties& PhysMatProperties, const FName& Name, const ETBBulletCaliber Type, const ETBProjectileSize Size, const float MuzzleVelocity, const ETBBulletVariation Variation, const float PenetrationValue = 1.f, const int32 ProjectileCount = 1, const FTBSpreadParams& SpreadParams = FTBSpreadParams())
		: BulletProperties(Properties)
		, PhysicalProperties(PhysMatProperties)
		, BulletName(Name)
		, BulletType(Type)
	...
	{
		return BulletName != NAME_None
			&& BulletProperties.CheckValid()
...
namespace TB::Bullets
{
	...
	namespace BulletOgives
	{
		...
		extern TERMINALBALLISTICS_API const FTBOgive Ogive_300WinMag;
	...
	namespace BulletPhysicalProperties
	{
		...
		extern TERMINALBALLISTICS_API const FTBBulletPhysicalProperties BULLET_300WinMag;
	...
	public:
		...
		static const BulletTypes& Get()
		{
			if (!StaticBulletTypes.IsValid())
			{
				Init();
			}
			return StaticBulletTypes;
		}
		...
		TMap<ETBBulletNames, FTBBullet> GetValidBullets() const
		{
			TMap<ETBBulletNames, FTBBullet> ValidMappings;
			for (const auto& Pair : BulletMappings)
			{
				if (Pair.Value)
				{
					ValidMappings.Add(Pair);
				}
			}
