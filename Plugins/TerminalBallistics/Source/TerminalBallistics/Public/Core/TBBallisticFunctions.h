...
namespace TB::BallisticFunctions
{
	...
	[[nodiscard]] FVector CalculateExitVelocitySimple(const FVector& SurfaceNormal, const FVector& ImpactVelocityMS, const double KineticEnergy, const double ImpactAngle, const double Mass, const double PenetrationValue, const double PenetrationThickness, const FTBSimplePhysMatProperties& SimpleProperties, const FPhysMatProperties& ObjectProperties, bool& bIsZero, double& dE, double& DepthOfPenetration, const double PenetrationMultiplier, bool bDebugPrint);
	...
	*/
	[[nodiscard]] FVector CalculateExitVelocity(const FHitResult& HitResult, BulletPointer& ActualBullet, const FVector& ImpactVelocity, const double PenetrationThickness, const FPhysMatProperties& ObjectProperties, bool& bIsZero, double& dE, double& DepthOfPenetration, const double PenetrationMultiplier = 1.0, bool bDebugPrint = false);
	static_assert(std::is_same_v<TB::BulletPenFunc, decltype(CalculateExitVelocity)>);

	[[nodiscard]] FVector CalculateExitVelocityLegacy(const FHitResult& HitResult, const FTBProjectilePhysicalProperties& Projectile, const FVector& ImpactVelocity, const double& PenetrationThickness, const TEnumAsByte<EPhysicalSurface>& SurfaceType, const FPhysMatProperties& ProjectilePhysicalMaterial, bool& bIsZero, double& dE, double& DepthOfPenetration, double PenetrationMultiplier = 1.0, bool DebugPrint = false);
	static_assert(std::is_same_v<TB::LegacyPenFunc, decltype(CalculateExitVelocityLegacy)>);
