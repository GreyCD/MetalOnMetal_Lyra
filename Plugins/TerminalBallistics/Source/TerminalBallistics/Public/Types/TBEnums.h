...
enum class ETBPenetrationComplexity : uint8
{
	None =			0,
	Simplified =	1 << 0,
	Complex =		1 << 2		UMETA(ToolTip = "Uses the physical properties of the projectile and the impacted object to calculate penetration.")
};
ENUM_CLASS_FLAGS(ETBPenetrationComplexity);
...
enum class ETBBulletCaliber : uint8
{
	...
	Bullet_12Gauge		= 37	UMETA(DisplayName = "12 Gauge"),
	None,
	Max							UMETA(Hidden)
...
{
	using enum ETBBulletCaliber;
	return BulletCaliber <= Bullet_357SIG || BulletCaliber == Bullet_12Gauge;
}
