...
namespace TB::Traits
{
	...
	template<typename T>
	struct TIsFVector<UE::Math::TVector<T>> { enum { Value = true }; };
	template<typename T> requires TIsDerivedFrom<T, FVector>::Value
	struct TIsFVector<T> { enum { Value = true }; };

	TB_TRAITS_CV_QUALIFICATION(TIsFVector);

	template<typename T>
	constexpr bool TIsFVector_v = TIsFVector<T>::Value;
	...
	template<typename T>
	constexpr bool TIsFVector2D_v = TIsFVector2D<T>::Value;
...
namespace TB::Concepts
{
	template<typename T>
	concept Vector2 = TB::Traits::TIsFVector2D_v<T>;

	template<typename T>
	concept Vector3 = TB::Traits::TIsFVector_v<T>;

	template<typename T>
	concept Vector = Vector2<T> || Vector3<T>;

	template<typename T>
	concept NetSerializable = requires(T obj)
	{
		{ obj.NetSerialize(std::declval<FArchive&>(), std::declval<UPackageMap*>(), std::declval<bool&>()) };
	};
};
