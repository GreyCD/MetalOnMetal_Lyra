...
public:
	...
	FTBProjectilePhysicalProperties()
	{
		if (CrossSectionalArea <= 0) {
			CrossSectionalArea = TB_SQR(UE_DOUBLE_PI * Radius / 100.0) + ((Length / 100.0 - 2 * Radius / 100.0) * Radius / 100.0);
		}
	...
	inline virtual double GetFrontalCSA() const
	{
		if (FrontalCSA <= 0.0)
		{
			FrontalCSA = TB_SQR((Radius / 100.0) * UE_DOUBLE_PI);
		}
		return FrontalCSA;
	}
...
public:
	...
	FTBProjectile(double InLength, double InRadius, double InMass, double InDensity, ETBProjectileSize InProjectileSize)
	{
		...
		CrossSectionalArea = TB_SQR(UE_DOUBLE_PI * Radius) + ((Length - 2 * Radius) * Radius);
	}

	virtual double GetSurfaceArea() const
	{
		const double SurfaceArea_Cylinder = 2.0 * UE_DOUBLE_PI * Radius * Length;
		const double SurfaceArea_Caps = 4.0 * UE_DOUBLE_PI * TB_SQR(Radius);
